...
// Modernisiert: ES Modules, vollständig
// =====================================================
...
// Mock-Daten (VOLLSTÄNDIG - 22 Spieler)
// WICHTIG: Korrekte Skalen - Form: 1-30, Motivation: 1-12, Strength: 1-10
const mockPlayers = [
    {
 ...
 * Basiert auf der exakten Kickerscup-Berechnung
 * * Korrekte Skalen:
 * - Spielstärke: 1-10
 * - Form: 1-30
 * - Motivation: 1-12
 * - Kondition (Stamina): 1-100
 * - Frische (Freshness): 1-100
 */
...
const showPlayerDetail = (playerId) => {
    ...
    // Wichtig: Fortschrittsbalken auf 100% skalieren
    // Stärke (max 10), Motivation (max 12), Form (max 30)
    const scaleStrength = player.strength * 10; // Max 100%
    const scaleStamina = player.stamina;        // Max 100%
    const scaleForm = (player.form / 30) * 100; // Max 100%
    const scaleFreshness = player.freshness;    // Max 100%
    const scaleMotivation = (player.motivation / 12) * 100; // Max 100%
...
const payTeamBonus = () => {
    if (confirm('Möchten Sie eine Teamprämie auszahlen? Dies erhöht die Motivation aller Spieler.')) {
        // Spieler-Motivation hat eine Skala von 1-12 (gemäß Kommentar im Code)
        players.forEach(player => {
            player.motivation = Math.min(12, player.motivation + 2); // Max auf 12 begrenzt
        });
        renderPlayers();
        updateTeamStats();
